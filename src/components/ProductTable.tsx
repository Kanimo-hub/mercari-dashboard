              ...
              {pageProducts.map((p) => {
                ...
                return (
                  <tr key={p.item_id} className="border-b last:border-0 hover:bg-gray-50 transition-colors">
                    ...
                    <td className="whitespace-nowrap px-4 py-3">{formatDate(p.listed_at)}</td>
        ...
        <div className="md:hidden divide-y">
          {pageProducts.map((p) => (
            <button
              ...
              className="flex w-full gap-3 p-3 text-left hover:bg-gray-50 transition-colors"
            >
              <div className="shrink-0 w-16 h-16 bg-gray-100 rounded-lg overflow-hidden">
                {p.photo_folder_url ? (
                  <img src={p.photo_folder_url} alt={p.title} className="w-full h-full object-cover" />
                ) : (
              ...
              <div className="flex-1 min-w-0">
                <h3 className="font-medium text-text text-sm line-clamp-2">{p.title}</h3>
                <p className="mt-1 text-base font-bold text-text">
                  {p.current_status === "取引完了"
